use engineering_metrics_data_collector::component::{merge_request, project, issue, external_issue};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let external_issue_tracker_enabled = env::var("EXTERNAL_ISSUE_TRACKER_ENABLED").map_or_else(|_| false, |val| val == "true");    
    ...
    for group_full_path in group_full_paths {
        ...
        if external_issue_tracker_enabled {
            let atlassian_rest_endpoint = env::var("ATLASSIAN_REST_ENDPOINT").expect("ATLASSIAN_REST_ENDPOINT environment variable is not set.").to_string();
            let atlassian_authorization_header = env::var("ATLASSIAN_API_TOKEN").expect("ATLASSIAN_API_TOKEN environment variable is not set.").to_string();
            let atlassian_jira_issue_url_prefix = env::var("ATLASSIAN_JIRA_ISSUE_URL_PREFIX").expect("ATLASSIAN_JIRA_ISSUE_URL_PREFIX environment variable is not set.").to_string();
            external_issue::import_external_issues(&atlassian_jira_issue_url_prefix, &atlassian_rest_endpoint, &atlassian_authorization_header, &updated_after, &store).await;
        } else {
