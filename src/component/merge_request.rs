...
impl MergeRequestHandler {
    ...
    pub async fn persist_closed_issues_on_merge(&self, issue: &ClosedIssueOnMerge) {
        let mut conn = self.context.store.conn_pool.acquire().await.unwrap();
        sqlx::query(
            r#"
    ...
    pub async fn import_closed_issues_on_merge(
        ...
        let closed_issues = self
            .fetch_closed_issues_on_merge(project_id, merge_request_id, merge_request_iid)
            .await;

        for issue in &closed_issues {
            self.persist_closed_issues_on_merge(issue).await;
        }
        ...
        if !closed_issues.is_empty() {
            println!(
                ...
                closed_issues
                    .iter()
                    .filter_map(|i| i.issue_iid.clone())
                    .collect::<Vec<String>>()
                    .join(","),
                &merge_request_iid,
                &project_id
    ...
    pub async fn fetch_closed_issues_on_merge(
        ...
        match group_data {
            Ok(data) => data,
            Err(_) => {
                self.context.gitlab_rest_client
                    .fetch_closed_external_issues(project_id, merge_request_id, merge_request_iid)
                    .await
